...
> **状态注(2026-07-20):本文已被 `factor-produce-groups.md` 取代,勿按此文
> 施工。** 实现走了同实证的另一条线:roster 入 PG 两表(替 manifest 文件 +
> 指纹闸门,等价校验是"DB 序 == XML 腿序")、组大小 500(本文议 200~300)、
> 新增待产态(新因子默认屏蔽)、封组 = bootstrap_groups --apply 生产根直接
> 全史(非 scratch mint 换入)、组 XML 建组时一次落盘(非运行时临时组装)。
> 保留本文作收敛前史:I1-I4 的实证动机与"代际累积/清淤/肇事腿隔离"三个
> 后议方向的原始表述仍有效(实施时以正主文档为准)。
