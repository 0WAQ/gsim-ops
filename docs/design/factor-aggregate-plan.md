...
| Wave 3 SSOT 收敛 | **部分** | 已做:Stage 表(S11)、`_batch`+CAS、now_iso(S12,`utils/clock.py`)、异常归因、xmlio/factor_dir、discovery_method 硬校验(S14 部分)、窗口命名常量(S13 半步,进 xml_prepare 未进 config)、~~FactorPaths(S4)~~(2026-07-09 阶段 1)、~~glob→LIKE(S9)~~(2026-07-09 阶段 2:`_glob_to_like` 转义+`?`→`_`+字符类跳过下推,`snapshot_where` 一处修)。~~metric 注册表(S8)~~(2026-07-11 小件收官批:`core/metrics.py::SNAPSHOT_METRICS`,SQL/内存/CLI choices 三方派生,`_METRIC_EXPR` 与 `_metric_get` 镜像删除);~~WRITE_COMMANDS 派生(S16)~~(2026-07-10 阶段 3 第二批:注册处 mark_write 声明) |
...
| `AlphaMetadata` | core/alpha/metadata.py | 保留(check 工作台视图);~~方法去 I/O~~ 2026-07-11 done:alpha_dump 扫描迁 services/check/checker/dumpscan.py,死代码 get_last_v1npy_file 删除(构造读盘解析 XML 属工作台语义,保留) |
...
- 验收:C3 → **0 红** ✅(9 条边全消,契约转 enforcing,6/6 绿;基线只剩
  ...
  (且升级为单事务原子);fast suite 42 passed ✅;PG 组 + 金丝雀环路
  ✅ **2026-07-10 于 160 验证全绿**(VERIFY-AGGREGATE-P2P3-RESULT:101 passed /
  e2e 6 passed / 金丝雀 4a-4g 全过,含 register 原子 stamped=t、rm 级联三表
  零行、cancel 级联零孤儿 info)。

### 阶段 3 · 命令塌缩 + 契约全绿(3-5 天)—— 第一批 2026-07-09 / 第二批 2026-07-10

- ~~archive/recall 收编(阶段 2 顺延)~~ ✅ 第二批(2026-07-10,前置已由
  VERIFY-AGGREGATE-P2P3 金丝雀全绿解除):`repo.archive` 收编 check.to_lib
  (搬运+@module 重指+pnl 分流+身份兜底断言)、`repo.recall` 收编 restage
  搬运半边、`repo.unstage` 收编 cancel/clear/rm 三处 staging rmtree;
  **S16 同批完成**(WRITE_COMMANDS 手抄删除,注册处 mark_write 声明派生)。
  `stage`/`iter_*`/`orphans`/`meta` 有意不预建(无消费者,W3 幽灵教训)。
  **合 main 前置:金丝雀环路在 phase3b 上复跑**(VERIFY-AGGREGATE-P2P3
  阶段 1/2/4)。
- ~~cli 层瘦身~~ ✅(`ops/cli/common.py` 单一接缝:re-export FactorStatus +
  STATUS_CHOICES + add_config_arg;14 个 cli 文件的 get_default_config_path
  手抄与 choices 收编;C2 契约 enforcing + 定点 ignore common 两条);
- ~~import-linter 全部转 enforcing,删基线脚本~~ ✅(**7/7 契约 enforcing**;
  contracts-baseline.toml / scripts/ci/import_baseline.py / CI ratchet step
  物理删除 —— ratchet 机制完成历史使命退役);
- 命令塌缩**部分**:status(repo.get / repo.find(include_submitted=True) 单条
  JOIN,退役双表内存合并)、cancel(迁 Repository;删除从"先 state 再 info"
  两步改 repo.delete 级联一步,消中间态泄漏窗口)、pack 的 -u/--status 过滤
  (退役 store.list+info.list 内存交集)✅;approve/restage/rm/info/list/
  backfill 已于阶段 2 迁毕。**剩**:submit/check 的进一步塌缩随 archive/recall
  批;"<20 行"核对 2026-07-11 完成,结论:
  8 命令的**单因子存储编排**全部达标(_clear_one≈2 / _cancel_one≈13 /
  backfill_one≈20 / _approve_one≈21,repo 一行式),`run_*` 入口仍 24-67 行 ——
  超出部分全部是批量骨架(_resolve_targets/确认交互)与 rich 渲染,不是存储
  编排残留;渲染半边归"展示层上收"(报告 Wave 4 未做项,独立中件)。
- ~~12→4 类型~~ ✅(live_table 展示类 FactorRow → LiveRow;全仓 grep 无
  FactorRow、无扫盘 FactorInfo;领域仅剩 Factor/FactorIdentity/FactorSnapshot/
  FactorRecord + 工作台 AlphaMetadata/FactorMeta/ScannedFactor 按计划保留);
- 验收进度:7/7 契约绿 ✅(160 实测 KEPT×7);金丝雀环路 + e2e ✅
  2026-07-10 于 160 全绿(VERIFY-AGGREGATE-P2P3-RESULT);三机滚存待合 main
  后窗口。**archive/recall 前置已解除**(金丝雀在含身份守卫 + Repository 的
  to_lib 上走通三次 archive:入库/拒绝归档/再入库)。
...
- AlphaMetadata 彻底去 I/O 若牵动 check 流水线过深,允许降级为"只挪
  get_v2npy_files 两个方法"并记账。(2026-07-11 按此降级执行:v2 两函数迁
  dumpscan.py + v1 死代码删除;构造读 XML 属工作台语义,不再追。)

## 附录 A · import-linter 契约(历史基线记录)

> **2026-07-09 阶段 3 起正主在 pyproject `[tool.importlinter]`(7/7 全部
> enforcing —— 现 8/8,C9 已加;C2 的 cli 接缝豁免点为 `ops/cli/common.py`;
> ratchet 基线 contracts-baseline.toml + import_baseline.py 已退役删除)**,
> 本附录仅保留为基线测量记录。
...
services 直用 rich(展示层上收,未做)。~~core 的文件 I/O~~ ~~`_METRIC_EXPR` 镜像~~(2026-07-11 小件收官批已清)。
(sudo WRITE_COMMANDS 手抄已由 S16 消灭 —— 2026-07-10 注册处声明派生。)
