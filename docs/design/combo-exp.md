...
/nvme125/production/combo/
  fguo/                    # 一个 combo(生产态)
    2020/ 2021/ … 2026/    # dump(SSOT,gsim 直写;含 weight/)
...
机制红利:`dumpAlphaDir` 指 combo **根**,gsim 自建 `<combo>/` 子目录 ——
dump 天然落在自包含位置;mode1/2 与 combo_eq 腿的 `alphaDir` 也同指 combo
根,引用关系极简。生成器:`scripts/build_combo_xml.py`(路径模板已按此布局)。
