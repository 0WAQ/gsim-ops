...
| REJECTED(compliance/correlation 失败) | 有 | 有 | 有 | 无 |
...
| check 失败 compliance/correlation (→REJECTED) | 保留 src | 保留 | 保留 | 无(check 不产 feature,仅 ops pack) |
| restage (ACTIVE→SUBMITTED) | 移入 staging(alpha_src 不再保留) | 保留 | 保留(--purge 清) | 保留(--purge 清) |
| restage (REJECTED→SUBMITTED) | 移入 staging(alpha_src 不再保留) | 清掉 | 清掉 | 清掉 |
| approve (REJECTED→ACTIVE, 仅 correlation 失败) | 保留 | 保留 | 保留 | 保留 |
| submit --overwrite (新代码, version+=1) | 新代码到 staging,旧 src 保留 | 保留 | 保留 | 保留 |

规则说明:
- restage 是 `shutil.move` 不是拷贝:召回后 staging 是 src 的**唯一副本**(cancel 因此用 entered_at 守卫拒绝曾入库因子)
- ACTIVE restage 保留产物:不暂停生产
- REJECTED restage 清掉产物(dump/feature/pnl,自动,与 --purge 无关):无生产顾虑,check 会重新产出
- submit --overwrite 保留旧产物:作为对比基准
- REJECTED 后两阶段失败保留 pnl + dump:数据完整,有分析参考价值(feature 只由 ops pack 产出)
- REJECTED 前两阶段失败不保留:短期数据不完整,无意义
- approve 是多样性豁免(为数据覆盖放行,非质量):仅适用于 correlation 失败,产物已完整,翻状态即可,无需重跑

## 版本控制

当前阶段:state record 加 `version` 计数器(`FactorRecord.version`)。检测历史记在
`factor_history` 全操作审计表(schema v2b:原 state 的 `check_history` JSONB 列已退役;
`HistoryEvent`/`CheckRecord` 均无 version 字段 —— 事件不按版本归档)。代码只保留当前
版本(不做历史快照)。
