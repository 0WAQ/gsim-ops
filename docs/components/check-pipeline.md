...
| 项 | 门槛 | 层 |
|---|---|---|
| 个股最大持仓 `max\|w\|/Σ\|w\|` | ≤ 5% | 软线 |
| 总持股数 | ≥ 100 | 软线 |
| 多头持股数 | ≥ 50 | 软线 |
| 空头持股数 | ≥ 50 | 软线 |
| 单日个股最大持仓 | ≤ `max_position_pct × hard_position_mult`(2× = 10%) | **硬顶** |
...
空/全 NaN/零敞口天跳过不计(缺数据的早期天天然免疫);软线任一违反记该日违规,
全史违规日 > `violation_tolerance`(10)才 REJECTED(放行早期毛刺);硬顶单日命中
