...
gsim 的 npy-load 模块 (`ProdNpyLoad` / `AlphaLoadFeat`) 在 `generate(di)` 里取的是
**前一天** 的信号: `self.alpha[valid_idx] = self.combo[di - 1, valid_idx]` (delay 防 look-ahead)。

因此 **combo.npy 的有效数据必须比回测/实盘的 startdate 早至少 1 个交易日**,
否则起点当天取 `combo[di-1]` 会落在 NaN 上 → alpha 全 0。后果按下游严格度分两种:

| 下游 | 首日 alpha 全 0 的表现 |
|---|---|
| `StatsSimpleV5` (simple/bench/layer) | 容忍, 当天静默空仓, 不报错 (易被忽略) |
| `StatsOptV5` + `AlphaOpRiskOpt20` 优化器 | **直接崩** `cvxpy ValueError: Invalid dimensions (0,)` (可优化标的数=0) |

**实测案例** (2026-06): combo predict `--start 20200102`, 回测也从 20200102 起,
优化器第一天 (20200102) 取 `combo[20191231]` = NaN → 全 0 → 崩。
放宽优化器约束 (maxPct/gamma/deltaind...) **无效** —— 约束只管"给定 alpha 怎么分配权重",
救不了"输入 alpha 全空"。改 startdate 往后挪是"绕过"非"修复"。

**正确做法**: predict 时 start 比目标回测 start 提前若干交易日 (覆盖 Decay 等 operation 的 warmup +
至少 1 天的 di-1 lag)。例: 回测要从 20200102 起, predict 用 `--start 20191201`。

> 这条之所以 combo 才暴露、alpha 不暴露: 普通 alpha 在 `generate` 里直接读 cc 行情 (有完整历史),
> 不存在"信号 npy 起点边界"; combo 的信号是预先 predict 的 .npy, 有人为起点,
> 所以 di-1 撞 NaN 是 combo 特有的坑。
