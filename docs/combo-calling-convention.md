...
> v0.2 (2026-06): 接口经 `tmp/combo_test/` 逐步最小化重建 + 实跑验证后定型。
...
| 概念 | 是什么 | 路径 | 谁的 | 数量 |
|---|---|---|---|---|
| **ComboProject 根** | 一个 combo 提交物的根目录 | `combo_<name>/` | qr 交付的**输入** | 一个 combo 一个 |
...
├── combo.meta.json        # 身份 + feature 声明 + 主产物名          [提交]
├── predict/               # 推理代码 (算信号, 入口 predict.py)       [提交]
├── models/                # 模型权重 (按日期命名的 checkpoint + feature_names.csv)  [提交]
├── config.simple.xml      # backtest config, 每个 stats 一份 (环境字段写占位符)  [提交]
...
└── runs/                  # ★ 所有产物统一出口 (提交时为空, ops/qr 跑测时生成)
    └── predict_<start>-<end>/ #   一次 predict (按区间命名, 滚动评估换区间=换目录)
...
**目录命名**:
- predict 层 = `predict_<start>-<end>` (`predict_` 前缀标明目录类型, 区间对滚动评估友好; 数据根/版本看 `combo.npy.meta.json`)
...
| 占位符 | ops 注入 | 性质 |
|---|---|---|
| `${RUN_DIR}` | predict 产物目录 (`runs/predict_<start>-<end>/`) | **目录前缀**, 撑任意多腿 |
| `${DATA_ROOT}` | 评估数据根 (cc_2025) | **目录前缀** (数据隔离核心) |
| `${START}` `${END}` | 评估区间 | 值 |
| `${PNL_DIR}` | backtest 输出 (`runs/predict_<start>-<end>/<stats>/pnl/`) | 目录 |
...
<Stats ... pnlDir="/ops/runs/<combo>_<date>/pnl"/>
<Alpha ... npydata="/ops/runs/<combo>_<date>/combo.npy" .../>
```
...
| 检查项 | 不通过 |
|---|---|
| `combo.meta.json` + `predict/` + `models/` + `config.<stats>.xml` (≥1 份) 齐全 | 拒收 |
| `predict/predict.py` 接受 data_root/start/end/device/output-dir | 拒收 |
| `combo.meta.json` 声明 `predict.main_output`, 且 predict 默认 out-name / config npy 引用与之同名 | 拒收 |
...
| predict warmup 起点早于回测起点 ≥1 交易日 | 警告 |
| `--device cpu` 跑通 predict | 警告 |

## 已验证场景 (tmp/combo_test/, 全部实跑)
...
| 占位符注入 (单腿) | ✓ |
| 多腿 + 优化器 (v0.2_pre 注入版, Hump×3 + RiskOpt20) | ✓ status optimal 全段 |
