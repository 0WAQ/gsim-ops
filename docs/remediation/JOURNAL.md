...
**Wave5 阶段 0 + 阶段 1 大半**(`docs/design/factor-aggregate-plan.md` 勾选为准):
...
## U11 · 共享 staging + PG 队列消费:代码/文档翻转就绪,待窗口(2026-07-11)

分支 `claude/shared-staging-queue`(已并入 main 的 setup 工程)。设计不变
(docs/design/shared-staging-queue.md:队列 = PG 状态表,数据面缺口 = staging 本机):
- `STAGING_IS_SHARED` 翻 True(staging 应然 = JFS 实目录;测试双模式钉住,
  False 分支防误翻);
- 部署手册 v2 重写(v1 写于 170 迁移前):阶段 0 已被 ops setup 工程消化,
  阶段 1 用 `ops setup` 参与切换(删共享软链一次全局生效 → setup 建实目录 +
  权限 → 各机搬 sidecar 存量 → 四机 probe),阶段 2 金丝雀跨机流转
  (160 submit → 170 check → 160 看 active → 异机 restage 回环);
- 全部 staging 相关文档(根 CLAUDE.md 软链约定/锁理由、config 注释、clear/
...
## ops doctor v1:盘 ↔ PG 数据对账落地(2026-07-12)

分支 `claude/ops-doctor-v1`。12 族对账挂账的收口工程(设计经三视角面板
安全/覆盖/MVP + 双评审综合;plan 已获用户批准,fix 范围与随批小件均经拍板)。

**七族**(注册表 checks.py,severity 在 kind 级):pool-ghost(classify 移植
自 reconcile_bcorr_pools.py,判定表不改语义)/ snapshot-stale(illegal 可修,
mismatch 只报告)/ info-orphan / src-drift / staging-drift(转介 clear/cancel,
不复制第二套删除逻辑)/ artifact-orphan(pnl/feature 只报告待 v1.1 判读放闸,
pack-tmp 残渣可修)/ dump-orphan(唯一 host scope,170 跨机 rm 残留的解)。

**三类低爆炸半径 fix**(+pack-tmp):池鬼影 unlink、非法快照 discard
(repo API)、本机 dump 孤儿 rmtree。全部经 guards.py **五道闸唯一出口**
(逐条 factor_lock → 锁内重验 TOCTOU → ACTIVE 绝缘 → realpath 白名单 +
src/pnl/staging 禁区 → 形态闸),判定 bug 最多"该删没删"不可能误删。
CLI:缺省纯只读零 sudo(不 mark_write);--fix 经 _FixAction 声明写性,
逐族独立确认(FixPlan 三句话逐字打印),非 TTY 无 -y 拒绝。

**显式拒收防回潮**:"ACTIVE 缺 dump"(health 结构性误报根源:dump 在消费机,
本机不可判)、dump 日期缺口、补数据类 fix、PG host 登记夹带。

**随批小件**:`scripts/postgres/migrate_snapshot_at.py` —— mismatch 侧
(ACTIVE 时间戳不符)的一次性 UPDATE(以 doctor JSON 名单为输入,dry-run
缺省);与 --fix snapshot-stale(illegal 侧)合计把 662 stale WARNING
刷屏两翼清零。doctor 本体不开 UPDATE 口子(快照不可变语义)。
...
**doctor v1 执行收官(2026-07-12)**:五阶段全绿(VERIFY-DOCTOR-RESULT,
8266320)。生产首轮对账基线:PG 8419 因子、七族五锚点精确命中(snapshot-stale
662 = illegal 642 + mismatch 20、pool ghost 0、info 孤儿 0、lib-missing 0)。
修复实战:illegal 642 经 `--fix snapshot-stale` discard 归零(无 locked/
vanished)、mismatch 20 经 migrate_snapshot_at.py UPDATE 归零 —— **`ops list`
的 stale WARNING 刷屏就此绝迹**(662→0),Total 8252 全程不变。170 本机面
dump-orphan 零残留(sidecar 解析正确;消费机尚未产 dump 属合法空态),160
共享面复验 setup --check FAIL 0。缺省只读零 sudo 5.2s 实证;非 TTY `-y` 的
材料前置确认流跑通(设计口子)。

**doctor v1.1 判读材料(留档 160:/tmp/doctor-160.json)**:
- ⚠ **107 个 src-orphan = 2026-07-06 迁移清理"108 脏因子"的盘面残渣**
  (当时纯 PG 删行,目录没动)—— **判读前全库禁跑 `ops backfill`**,会把
  被判脏的因子整批复活成 ACTIVE 入库;处置方向大概率是人工核对后删目录,
  不是补录;
- artifact-orphan 62 feature-orphan + 4 alien:pnl/feature 孤儿放闸
  (unlink 升格 fixable)的判读输入;
- pool-ghost 8 missing:approve 豁免/瞬态,合法,无动作。

## doctor v1.1:四批孤儿判读 + feature 放闸 + L1 写入侧修复(2026-07-12)

分支 `claude/doctor-v11`。v1 基线的 173 条 WARN(107 src-orphan + 62 feature-
orphan + 4 alien)经两轮细核(DOCTOR-V11-TRIAGE)重划为**四批独立残留**:
D 4 alien 临时垃圾(正式 npy 全在位)/ B 26 命名迁移 feature 孤儿(长名在库)/
C 5 ybai 双命名两面(长名 src + 短名 feature,doctor 因命名严格比对拆成两条)/
A 107 src-only 残留(完整目录、PG 零记录、零产物侧影;4 个 mtime 最新的 ybai
逐一复核均为死孤儿)。用户拍板:feature 走 doctor 放闸、A 批直接脚本删。

- **feature-orphan 放闸**:artifact-orphan 族的 fixer 升级为按 kind 分派
  (pack-tmp mtime 复验 / feature-orphan 锁内 PG 重验 + 文件名合式复验),
  FixPlan 动作面同步改写;pnl-orphan 仍只报告(基线 0,无判读材料);
- **`scripts/cleanup_src_orphans.py`**:A 批一次性名单化处置(doctor JSON
  输入,dry-run 缺省;逐目录 factor_lock + 锁内 PG 复核 + staging 排除 +
  非软链真实目录 → rmtree)。**alpha_src 不进 doctor 删除集的铁律不破** ——
  这是判读后的一次性通道,Phase D git 化前不常规化;
- **L1 写入侧修复**:factormeta XML author 回退路径小写归一(Fguo/fguo
  同人分裂的写入口)+ submit_one birthday 区间校验(20150101-20991231,
  仅"给了但离谱"拒收,缺省 0 放行 —— backfill 存量无此字段);
- L2(ybai 长/短双命名致配对假阴性)只档案不写代码(一次性历史,启发式
  归一有误配风险);
...
僵尸列 / 类型选择),多轮讨论收敛为 `docs/design/schema-v2.md`:改什么、为什么、
...
分支 `claude/schema-v2b`(基于 schema-v2)。设计见 docs/design/schema-v2.md,用户已批。
...
**v2b 生产迁移收官(2026-07-12,VERIFY-SCHEMA-V2B-RESULT)**:五阶段全绿,
...
分支 `claude/schema-v3`,设计 docs/design/schema-v3.md(用户深度参与,快照语义变更
...
## legacy 老因子档案清理批(2026-07-13,当日全线闭环)

分支 `claude/legacy-cleanup`,设计 docs/design/legacy-cleanup.md,手册
VERIFY-LEGACY-CLEANUP.md。输入 = v3 收官暴露的三笔存量账:472 条 snapshot_at
漂移、compliance 22 条无快照、discovery_method 129 条 NULL。

**拍板**(用户):discovery_method **不允许 NULL**,值按 pnl 分流池位置判,
两池不能判的人工点名(ybai/zxu/cchang/sli → manual,hwang → automated,
fguo 2 条点名 automated);`ops backfill` 退役删除;list 混排加 status 列。

**代码批**:三迁移脚本(全部本地沙盘实测:守卫触发回滚/幂等多跑/新直连
复核)+ backfill 退役(HISTORY_OPS/chk_op 保留历史枚举)+ doctor 第八族
timeline-drift(created_at <= submitted_at 不变量)+ discovery NOT NULL
三处 DDL 同改 + `_ensure_record` 无 dm 显式拒绝 + status 列。

**生产执行**:①472 全拉正(事务自验残余 0)②22/22 补跑落库 ③129 归一
(automated 8035 = 8015+20 / manual 384 = 275+109 严丝合缝)+ chk_discovery
收窄 + SET NOT NULL 收口 ✅;阶段 6 池补账 31 条(执行者会话 sudo 无 TTY,
0/31 落地被其新直连核对抓获停机 —— 用户 sudo 终端补跑落地);全量 doctor
8 族 0 fail,pool-ghost missing 39→8(降幅恰 31,余 8 条 approve 豁免合法)。

**执行期证伪与新知**:发现①"zxu 40 条 NULL 疑似旧部署提交"**不成立** ——
129/129 submit 事件 actor=migration(v2b 合成存量),submit 硬校验一直有效;
执行者抓获 `uv run ops` fall-through 全局旧 shim(项目 not packaged)——
手册验证路径改 `uv run python -m ops.main`,并确立纪律:**未合并分支绝不
reinstall 成全局命令**,生产工具跟 main 走。
