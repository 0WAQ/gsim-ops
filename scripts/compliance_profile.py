...
def profile_one(npz_path: Path, dates: np.ndarray, args) -> dict:
    ...
    row.update({
        ...
        "viol_head252": int(v_any[:args.head].sum()),
        # v_any[-0:] 是整个数组(Python 切片陷阱),tail=0 显式归零
...
def main() -> int:
    ...
    n_meta_only = len(meta) - len(rows)
    print(f"\n输出: {out}({len(rows)} 行;summary 有行但无 npz 的 "
          f"{n_meta_only} 个 = 零有效日/出错因子,天然无画像)")
    _report(rows)
    return 0
