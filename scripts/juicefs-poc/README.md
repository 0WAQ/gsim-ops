...
│                                (本来就有, JuiceFS 不动)
├── redis-jfs.service     :6380  JuiceFS metadata only
│   ├── AOF on (appendfsync everysec, 重启不丢)
│   └── 独立 conf/data dir, 进程级隔离 (6379 重启不影响 6380)
├── JuiceFS client (FUSE) ───────────────────────→
...
06-redis-jfs.sh     起独立 redis 实例 (port 6380, AOF on, 进程级隔离)
...
| `JFS_META_URL` | `redis://127.0.0.1:6379/0` | 跑过 06-meta-migrate 后覆盖为 `redis://<host>:6380/0` |
| `JFS_ENV_FILE` | `/etc/juicefs/<name>.env` | 06-meta-migrate 后覆盖为 `<name>-jfs.env` |
| `JFS_REDIS_LOCAL` | `1` | 0 = unit 不依赖本地 redis(给 client 用) |
...
# config.juicefs.yaml 已经设了 state.backend: redis, 之后 ops 自动走新 store
```
...
export OPS_STATE_REDIS_PASSWORD=$(sudo grep -oP 'META_PASSWORD=\K.*' /etc/juicefs/alphalib.env)

# 本机 alphalib 路径覆盖 (config.juicefs.yaml 的 vars.alphalib_root 默认是主节点的 /tank/vault/alphalib)
export OPS_ALPHALIB_ROOT=/storage/vault/alphalib   # 144 这种磁盘布局
# 160 / 150 不需要这条, 它们路径跟 config 默认一致

uv run ops list -c config.juicefs.yaml | head
```

永久化建议:写个 `~/bin/ops-jfs` wrapper 自动 export 两个 env,后续直接 `ops-jfs list -c config.juicefs.yaml`。
...
- [x] Redis 网络化 + AUTH + AOF(`03-redis.sh`,改成运行时 config set 避雷)
- [x] Client 节点接入(150 实测通过 `join.sh`)
- [x] 跨节点可见性 + flock 真锁验证(2026-06-04)
...
- [x] **`ops` state 后端切到 redis**(2026-06-04,`ops/infra/store/redis_store.py` + `state.backend: redis` 配置 + `ops/tools/state_migrate.py` 一次性 JSON→redis 迁移,3224 records 迁完,跨节点强一致)
- [ ] Redis Sentinel(需要第三台 sudo 节点)
- [ ] 全量数据迁入(160 已完成,后续节点按需迁)
- [ ] `LibraryScanner` index/metrics/bcorr cache 进 redis(跟 state 同思路,目前还是 per-machine `~/.cache/ops/lib/`,跨节点 first-scan 慢)
...
**2026-06-04 之后**:`config.juicefs.yaml` 已经把 state 切到 redis,所有节点共享同一份 state。任何节点跑 list 都能看到颜色和 `fail_stage`,**前提是**:
...
现象:`ops list -c config.juicefs.yaml` 抛 `redis.exceptions.AuthenticationError: HELLO must be called...` 或 `ConnectionError`。
