...
```
_lib.sh         公共自检 helper (sudo / 二进制 / systemd / TCP / mountpoint),source 用
config.sh       配置 + 凭证解析 (rclone.conf 回退),source 用; `--show` 预览
bootstrap.sh    主节点一键部署,分 5 个 stage
verify.sh       PoC 验证套件 (basic / memmap / git / redis-fail)
join.sh         Client 节点一键接入
teardown.sh     卸载 (--purge 才真删数据)
verify_memmap.py  alpha_feature memmap 仿真脚本,verify.sh memmap 调
```
...
| `JFS_CLIENT_ONLY` | `0` | 1 = `bootstrap.sh install` 跳过 redis |

**MinIO 凭证只在 `bootstrap.sh provision` 阶段需要**。挂载、AUTH、跨节点全程不用。
...
# 或分步(每步幂等)
sudo -E bash bootstrap.sh install      # redis + juicefs
sudo -E bash bootstrap.sh provision    # MinIO bucket + format + 临时挂
sudo -E bash bootstrap.sh layout       # sidecar symlink + 两组权限
sudo -E bash bootstrap.sh redis        # redis 网络化 + 生成密码
sudo -E bash bootstrap.sh systemd      # systemd 接管挂载
...
- `recycle` 嵌套一层 unixId,`layout` stage 按现有子目录名 `chown <unixId>:<primary>`
...
- [x] sidecar 改 symlink(`layout` stage)
- [x] 权限模型(setgid + umask,无 ACL)
- [x] `/etc/profile.d/ops-umask.sh`
- [x] systemd unit 接管主节点挂载
- [x] Redis 网络化 + AUTH
...
- `rclone.conf` 的 `no_check_bucket = true` 让 `rclone mkdir` 假成功;`provision` 加真 PutObject 兜底
...
- 改 redis.conf 前必须先 stop juicefs unit,否则现挂载在 AUTH 切换瞬间全 EIO。`redis` stage 已自动停
- `usermod -aG` 不影响已有 SSH session;验证用 `sg <group> -c <cmd>`,或重连
- 密码出现在 `juicefs mount` 的 cmdline 会被 `ps` 看到;`systemd` stage 通过 `EnvironmentFile=` 注入 `META_PASSWORD`,URL 不带密码
