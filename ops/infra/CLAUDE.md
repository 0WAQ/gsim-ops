...
原子写**(store 的 `upsert_on`/`put_on` 静态方法组合;submit/backfill/
check._ensure_record 的唯一双表写入口)/ `record`/`transition`(CAS 透传)/
`append_check` / `attach_snapshot`(强制 `snapshot_at = entered_at` + stale
自愈)/ `discard_snapshot`(离库快照失效)/ `delete`(info 级联)/ `exists` /
`lock`(factor_lock 门面)。
...
- `pg_store.py` — `PostgresSnapshotStore`,`factor_snapshot` 表(`id SERIAL` 主键,`name UNIQUE`,外键引 factor_info)。GIN(fields/tables) 反查、ret/shrp B-tree 索引。`list(...)` 把 field/tables/metrics/sort_by/limit 拼成 WHERE/ORDER BY/LIMIT 下推 SQL(承接原 DerivedStore.get_all 的下推语义;metric 键 SQL 表达式自 `ops/core/metrics.py::SNAPSHOT_METRICS` 注册表派生,2026-07-11 S8 收敛,原 `_METRIC_EXPR` 手抄映射删除)。has_pnl/dump_days 代码侧已删(⚠ 生产删列迁移 migrate_drop_snapshot_index_cols.sql 至 2026-07-12 从未执行,v2a 补执行 —— 用户查活表发现,此前文档'已删列'口吻失实);list 因子集判据 = `factor_state.status != 'submitted'`(2026-07-07 Wave 2,纯 PG 零扫盘;见 `query.py`)。删列迁移 `scripts/postgres/migrate_drop_snapshot_index_cols.sql`。
