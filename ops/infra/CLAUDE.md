...
(clean_pycache + src→alpha_src + @module 重指 + **生产化 productionize_src** +
dump/pnl 搬库 + 按来源分流池副本 + 身份兜底断言,第一道闸仍在 check.run_one
入口)/ `productionize_src(name)` —— 归档 XML 生产化(core/prodxml 三张规则表)
+ 作废产线 checkpoint(重入库 = 代码已换;pass 与 REJECTED 归档都走 ——
approve 不经重检直接翻 ACTIVE;produce 块缺失警告跳过,残缺响亮抛;
factor-produce-v3.md D9/D10)/ `recall(name)` ——
...
- `base.py` — `FactorInfo` dataclass(`core/factor.py::FactorIdentity` 的别名,name / author / discovery_method / created_at)+ `InfoStore` ABC(`get` / `upsert` / `delete` / `list(author=...)`)
...
- `run_backtest(xml_path, config)` — runs gsim backtest, raises `BacktestError` on failure。
  置 `PYTHONDONTWRITEBYTECODE=1`:@module 指共享盘(staging/alpha_src)成为常态,
  root 不往 JFS 写 `__pycache__`
