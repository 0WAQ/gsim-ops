...
# 不在 record 上:按需 store.checks() / repo.history()。
# lf.* 来自 factor_history 派生(state 的 rejected_at/last_fail_* 已删列)。
_FIND_COLS = (
...
class FactorRepository:
    ...
    def record(self, name: str) -> FactorRecord | None:
        """state 轻量读(纯状态机快照,不含 check 全史),不拼 Factor。"""
    ...
    def find(
        ...
        **因子集判据**(JOURNAL V1):库内因子 =
        ...
        # factor_state 也是 LEFT JOIN(原 INNER):info 有行、state 无行的孤儿
        # (register 事务化前的半截写入 / 手工残留)在 include_submitted=True 的
        # "任何记录"语义下必须现形(status 全列表 / cancel 批量的"需对账"提示),
        # 否则对账线索为零。缺省因子集判据 `s.status != 'submitted'` 对 NULL
        # 求值为假 —— 孤儿天然不算库内因子,ops list 不受影响。
        query = f"""
            SELECT {_FIND_COLS}
            FROM factor_info i
    ...
    def register(
        ...
        *,
        ...
        调用方(submit/check._ensure_record)在 factor_lock 内先判不存在再调;
        info 是 upsert、state 是 put(upsert),崩溃重跑幂等。op 发射命令事件;
        status=ACTIVE 另发 'entered'(入库统一标记,与 transition 的自动发射同规)。
    ...
    def attach_snapshot(self, snapshot: FactorSnapshot,
        ...
        """测得快照落库(factor_snapshot = 最近一次 check 测得的表现,被拒也
        写)。snapshot_at = measured_at(该次 check 事件的时刻,由 check 流水线
        传入);入库见证已全权归 entered_at/entered 事件,原 entered_at 硬闸
        删除 —— 写快照 ≠ 入库(词汇表)。

        每次测量原子替换:旧行让位,否则 insert 撞 name UNIQUE 被吞,读侧永远
        停在上一次测量。json dev/test 后端无快照表,尽力而为 no-op。
        """
        if not self._is_pg:
            return
        if not measured_at:
    ...
    def delete(self, name: str, op: str | None = None) -> bool:
        ...
        factor_history 无 FK,是唯一活过删除的痕迹。
    ...
    def archive(self, name: str, *, src_dir: Path, dump_dir: Path,
        ...
        """staging/工作区产物归档入库(收编原 check.to_lib):
    ...
    def recall(self, name: str) -> None:
        ...
        move + @module 重指。**move 不是 copy** —— 召回后 staging
