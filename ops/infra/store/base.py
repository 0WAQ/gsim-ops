...
# 定义已迁 ops/infra/errors.py(类型化异常集中地);此处 re-export 保住
...
class StateStore(ABC):
    ...
    # author 参数已删:三表拆分后 FactorRecord 不再有 author,PG 实现只收
    # status(ABC 与实现签名漂移 = LSP 违反)。author 过滤走 InfoStore.list(author=...)。
    @abstractmethod
    ...
    def transition(self, name: str, to_status: FactorStatus,
        ...
        StateConflict(from-status 守卫,防止任何状态被翻成任何状态)。

        op 非 None 时同事务发射 factor_history 事件;此外 to_status == ACTIVE
        时**无条件**自动发射 'entered'(入库事实的统一标记,check 归档 /
        approve 等入库路径合流,漏记结构上不可能)。
    ...
    def checks(self, name: str) -> "list[CheckRecord]":
        """check 全史(自 FactorRecord 剥离,按需查)。PG 后端从事件表
    ...
    def last_fail(self, name: str) -> HistoryEvent | None:
        ...
        原 factor_state 三列 rejected_at/last_fail_* 的派生替身。
        json 后端从 check_history 内存扫描合成。None = 从未失败。"""
        ...

    @abstractmethod
    def latest_check_ats(self) -> "dict[str, str]":
        """全库 name → 最近一次 check 事件的 at(doctor 用它对账测得快照 ——
        snapshot_at 应等于最近一次测得的 check 时刻)。
