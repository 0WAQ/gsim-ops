...
Example:
    ...
    parser.add_argument("-u", "--user", type=str, required=True, action=LowerAction)
    parser.add_argument("-s", "--start-date", type=str, required=True)
    parser.add_argument("-e", "--end-date", type=str, default=None)

    parser.add_argument("--dropbox-path", type=Path, default="/mnt/storage/dropbox")
    parser.add_argument("--target-path", type=Path, default="/mnt/storage/work/wbai/check_bias")
