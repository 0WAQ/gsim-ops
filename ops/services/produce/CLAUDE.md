...
`ops produce --grouped` = 三态产线:**组产**(roster 在 PG produce_group 两表)
+ **单产**(注册表 produce_single,"组大小为 1 的组",补丁式 XML)+ **待产**
(纯推导,只报告永不生产 —— 新到因子默认屏蔽)。sync 做 DB/XML/ACTIVE 三方
收敛:组侧静音/解静音/不变量校验(DB 序 == XML 腿序)、漂移自动降级转单产;
单产侧离库移除、漂移重冻结(删 checkpoint 全段重跑)。设计正主
`docs/design/factor-produce-groups.md`(状态机/转移表在 §4)。
建组走 `scripts/bootstrap_groups.py`(dry-run 出样品,--apply 落盘写库,
封组 = 单产转正的通道)。当前生产闸 delay==1(可生产域 = 全部 ACTIVE)。

测试:`tests/test_produce_groups.py`(json 后端 + FakeRepo + fake backtest:
三态 sync/准入/运行/退出码)、`tests/test_group_store_pg.py`(PG roster +
单产注册真身)、`tests/test_prodgroup.py`(纯函数:划分决定论/XML 两种生成)、
`tests/test_runner_logs.py`(产线日志落盘)。
