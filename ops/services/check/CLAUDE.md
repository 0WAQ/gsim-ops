...
**快照落库时机**:主路径(pass→archive)四组齐全(index 组延后扫盘补)。REJECTED 因子(`on_reject`)**不写 snapshot**(未入库);快照不可变,无运维补救路径(旧 `ops refresh` 已删)。
...
**Failure semantics**:
- validate / long_backtest fail → revert to SUBMITTED, factor stays in staging (environmental/config issue,下次 ops check 自动重扫)
