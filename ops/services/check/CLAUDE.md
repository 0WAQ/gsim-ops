...
路径现居 `repo.archive` 内,2026-07-10,restage 侧同款在 `repo.recall`)唯一负责。XML 读写统一走 `ops/utils/xmlio.py`(unparse 参数
只写一处)。
