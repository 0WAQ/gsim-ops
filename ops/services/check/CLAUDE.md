...
unexpected-error 臂接住 → revert SUBMITTED + 完整日志。`prepare_for_archive`
(拆雷)已于 2026-07-18 退役 —— 归档 XML 现在**就是生产态**(`repo.archive` →
`repo.productionize_src` → `core/prodxml.py`,入库即适配生产线,
factor-produce-v3.md D9;"防手动重跑砸库"改由禁直跑 + 未来 ops export 承担)。
@module 由搬完目录后的 `rewrite_module_path`(`ops/utils/factor_dir.py`;归档
路径现居 `repo.archive` 内,restage 侧同款在 `repo.recall`)先重指,
生产化再改成跨机稳定前缀(D11)。XML 读写统一走 `ops/utils/xmlio.py`(unparse 参数
只写一处)。
...
快照永远停在旧代码(full-review P0-1)。正常路径下旧行已被 restage/--overwrite 删除。
**`to_lib` 已变薄**(2026-07-10 阶段 3 第二批):搬运 + @module 重指 + 按
`discovery_method` 把 pnl 分流到 `pnl_automated/` 或 `pnl_manual/`,全部收编
...
`repo.archive` 兜底 RuntimeError(走 unexpected 臂;断言随搬运收编 Repository,
2026-07-10)。archive 的 move 落点显式锚定 `paths.src`/`paths.dump`(与 rmtree/
rewrite 同锚点)。用例
