...
- `diff(local, remote, deep_root=None)` → `DirDiff` 四类清单:`only_local / only_remote / differ / identical`。
...
`newer_side(rel, d)` → `local / remote / tie`。`MTIME_TOLERANCE = 2s` 吸收文件系统量化和 S3 LastModified 抖动。

### mtime 校准

S3 LastModified 是**上传时间**,本地文件 mtime 是**编辑时间**——不校准的话,刚 pull 下来的文件 mtime > 远端 mtime,下一次 push 会被误判 differ 反推。

为消除歧义:`_push_dir` 上传成功后 `head_object` 拿 `LastModified`,`_pull_dir` 直接拿 list 里的 mtime,统一 `os.utime(local, (lm, lm))` 把本地 mtime 校准到远端 LastModified。这样两端 mtime 永远在 `MTIME_TOLERANCE` 内可比,**只有真正的本地修改才会让 local.mtime > remote.mtime**。
...
`ops sync verify` 实跑:对三个数据目录两端列举 → 输出 `only_local / only_remote / 大小不一致` 三类清单。只读。
`ops sync verify --deep`:在上面基础上,对所有 `identical` 文件本地复算 etag 跟远端比,捕捉等大小内容漂移。代价是读全部本地文件(~500MB/s),`alpha_feature` 482GB 全跑约半小时。

## Operations

- `ops sync push` — list+diff 文件级增量推送 + state merge
- `ops sync push --deep` — 同上,等大小再走 etag 比对(用于已知发生过原地重写但不放心 mtime 的场景)
- `ops sync pull` — state merge + list+diff 文件级增量拉取(按 status 过滤)
- `ops sync pull --deep` — 同上,等大小再走 etag 比对
- `ops sync status` — 不扫数据目录,只对比两端 state 总数 / 仅本地 / 仅远端 / 远端更新数
- `ops sync verify` — 三个数据目录文件级两端校验
- `ops sync verify --deep` — 上面基础上加 etag 校验(慢)
