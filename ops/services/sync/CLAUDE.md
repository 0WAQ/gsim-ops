...
mtime 不再参与 identical/differ 分类。同 mtime+size 不同内容的边缘场景由 `--deep`(强制重算)兜底。
...
`newer_side(rel, d)` → `local / remote / tie`,**仍按 mtime** 比较——这是 `differ` 时唯一的方向信号。`MTIME_TOLERANCE = 2s` 吸收文件系统量化和 S3 LastModified 抖动。

### 方向判定的已知局限

`newer_side` 用 mtime 选谁覆盖谁。校准后本地 mtime 是**上次 push 的 LastModified**,不是"本地真实生产时间"。在以下场景方向会错:

- 跨机器独立生产同一文件、push 顺序乱:晚生产的反而先 push,会被后 push 的覆盖
- 本地刚 pack 完没 push,另一台机器先 push 了别的版本:pull 会用远端覆盖本地未推送的新产物
- 本地 `cp -p` / `touch` / rsync:mtime 不反映内容时间

要彻底解决需在 `.state/` 维护 `data_manifest.json` 用 `produced_at` 选方向(方案 B),工程量大,已 defer。当前接受这个局限。

### mtime 校准

S3 LastModified 是**上传时间**,本地文件 mtime 是**编辑时间**。校准的目的从"让 mtime 可比"退化为"让 etag 缓存命中":如果 push 后本地 mtime 没被校准,下次 walk 时 mtime 不等于缓存里的 mtime,etag 会被重算——纯粹浪费 CPU。
