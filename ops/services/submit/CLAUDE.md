...
One-shot for legacy factors in `alpha_src/` (originally 2194, now 7594 in prod 三表迁移后) — builds the npy_index once and reuses it across all `parse_factor()` calls (the optional `npy_index` param avoids redundant filesystem walks). **经 `repo.register` 原子写 `factor_info`(author + discovery_method,缺省 `backfill`)+ `factor_state`(不带 author,已入库因子 status=ACTIVE,`entered_at`=回填时刻)**。Skips records that already exist in state.
