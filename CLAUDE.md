...
- 2026-07-18 因子增量生产 v3 接管序列 (branch `claude/factor-production-features-pdnjdc`): 归档即生产态落地后,存量 8297 归档 XML 一次性生产化 (`scripts/migrate_prod_xml.py`, 0 失败, 备份在位) + 影子对拍 20/20 byte-equal (150800 文件, `scripts/produce_shadow_diff.py`) + 单因子金丝雀五点验证全过。545 条"在产不在库"腿裁决 = combo 摘腿 (不补入库)。接管后续步 (权限交接/cron) 被 v4→三态演进取代。见 `docs/design/factor-produce-v3.md` + `docs/remediation/MIGRATE-PROD-XML-RESULT.md`。
- 2026-07-19/20 分组产线三态 (`ops produce --grouped`, 设计 `docs/design/factor-produce-groups.md`): **在产 = 组产 (roster 入 PG produce_group 两表, 组 XML 腿序冻结) + 单产 (produce_single 注册表, "组大小为 1 的组", 补丁式 XML); 待产 = 可生产 − 在产, 纯推导, 新到因子默认屏蔽永不生产**。组大小 500、sibling 平铺共享 init (日常提速)、产物落 `/nvme125/production/alpha`、生产闸 delay==1。CLI: 缺省 = 组产+注册单产; `--groups-only` / `--single-only [names]` (点名 pending 先准入)。170 真机试点 (2 组×5 腿) 75460 dump + 10 pnl 与旧 dataset 全部 byte-equal。实证 `docs/remediation/BATCH-PRODUCE-MECHANICS-RESULT.md`。
- 2026-07-20 combo 产线立项首跑 (`/nvme125/production/combo`, 设计 `docs/design/combo-production.md`): 4 combo (fguo/lhw/zxu + combo_eq 聚合) × 3 mode (mode0 计算 / mode1,2 直读 dump 不重算), 全员 Combo_su10 + TOP3000 + 全 mode 中性化 + 中证1000基准, startdate 20200101 (2011 因 lhw/zxu 数据全 NaN 被 LGBM 零样本拒, 回现役口径)。XML 全部生成器产物 (`scripts/build_combo_xml.py`, 勿手写)。首跑 (窗→20260710) mode0×3 全过, 产物计数与腿数精确吻合。同日: author 大小写归一迁移 (`migrate_author_case.sql`, Fguo→fguo 848 行) + `ops list --filter-by` 支持 delay (SNAPSHOT_METRICS 注册表一行)。
- 2026-07-19/20 AI 工具链迁移 Claude Code → Kimi Code: 指令三层 —— 全局 memory 软链 (`~/.kimi-code/AGENTS.md` → 仓库 `.claude/memory/MEMORY.md`, `.claude/link-memory-kimi.sh` 幂等, memory 本体 git 滚存) + 项目级 `AGENTS.md` (/init 生成, 已入库) + skills/agents 移植 (`.kimi-code/skills` 软链 `.claude/skills`; 6 个只读 agent 转角色 skill, kimi 无自定义子代理概念)。分支 35 笔合 main (含 claude 侧 7-18 三笔: 545 名单/fd 自举/v4 设计; v4 已标被取代, 差异入正主)。
