...
QR 团队提交 combo 给 ops 用最新数据 (含样本外 cc_2025+, QR 无权访问) 代测。接口规范 2026-06 定型并实跑验证, 文档在 `docs/design/combo/*.md`。
...
**已实现 (2026-07 注)**: `ops combo` 子命令已落地 (ops/cli/combo.py + ops/services/combo/)。以下为原始 spec 未来扩展项的时点记录: `ops combo` 子命令 (读 config→替占位符→算 H→跑 gsim, 当时手动 sed 模拟) / 多 combo 批量评估排名 / 样本内优化器 API (QR 自助查样本内优化后表现, 绝不返回样本外) / 依赖链。
...
**已纸面验证** (玩具模型, `tmp/ComboToyRolling/`): ops 滚动调 train.py 训 4 季度 checkpoint → predict 按日期季度切 → backtest, 三段端到端通, 纯 CPU。**未验**: lhw 真实训练代码 (在 DeepStock-E2E, 未交) 能否套入 / 真实算力。规范产出 `docs/design/combo/train-spec.md` (给 qr 的训练代码提交要求), 待拿去要真实 train.py。
