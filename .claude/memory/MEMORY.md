...
- [Combo Test Pipeline](project-combo-test-pipeline.md) — combo 测试接口规范定型: QR 交 predict+models+config.<stats>.xml, ops 用最新数据代测; 占位符注入消除硬编码, 主产物固定 combo.npy, predict/backtest 逻辑分离, warmup 坑; 文档 docs/design/combo/*.md, ops combo 子命令已实现 (ops/cli/combo.py + ops/services/combo/)
